...
func (r *GroupMemberRepo) GetByMemberID(ctx context.Context, memberID int) ([]domain.GroupMember, error) {
	start := time.Now()
	...
	if err != nil {
		metrics.ObserveDB("group_member_get_by_member_id", time.Since(start), err)
		return nil, err
	}
	defer rows.Close()
